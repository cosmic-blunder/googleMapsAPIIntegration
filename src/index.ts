...
function UpdatePanel(legs:google.maps.DirectionsLeg[]){
  ...
  if(panel)panel.innerHTML="";
               legs.forEach(leg=>{
                      if(panel)
                        {
                          panel.innerHTML+="("+num+++") Distance:"+leg.distance?.text+"</br>"+"time:"+leg.duration?.text+"</br></br>";
...
function addMarker(position: google.maps.LatLng | google.maps.LatLngLiteral) {
  ...
  if(marker_.length>1){
    ...
    glob.dS.route({
      ...
      waypoints: markers.slice(1, markers.length-1).map<google.maps.DirectionsWaypoint>((pos)=>({ location: pos.getPosition()! })),
...
function RemoveMarker(this: any, e){
      ...
      markers.forEach(m=>{
        if(m===this){
            ...
            markers = markers.filter(m=>m!=this);
...
function UpdatePath(){
  ...
  glob.dS.route({
    origin:markerCar?.getPosition()!,
    destination:markerCar?.getPosition()! ,
   //waypoints: markers.slice(1, markers.length-1).map<google.maps.DirectionsWaypoint>((pos)=>({ location: pos.getPosition()! })),
    optimizeWaypoints: true,
    travelMode: google.maps.TravelMode.DRIVING,
  })
